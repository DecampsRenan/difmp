...
- `path: PathInput = \`/${string}\` | "_"`. `"/x/_"`is registered for both`/x/*`and`/x`.
...
`Layer.provide` on a _route_ layer does not satisfy the marker and does not error. Provide **after
...
const CorsLayer = HttpRouter.cors({
  // global, verified 204 + ACAO on preflight
...
`serve`/`toWebHandler` wraps the _whole_ chain including response writing — response mutations there are lost;
...
const lines =
  yield *
  fs.stream(p, { chunkSize: 8192 }).pipe(Stream.decodeText(), Stream.splitLines, Stream.runCollect);
```
...
```ts
yield *
  Stream.fromArray([new Uint8Array([1, 2, 3])]).pipe(Stream.run(fs.sink(target, { flag: "w" })));
```
...
const SseRoute = HttpRouter.add(
  ...
  Effect.gen(function* () {
    ...
    ).pipe(Stream.map(render), Stream.encodeText, Stream.ensuring(Effect.log("sse client gone")));
...
export const MainLayer = HttpRouter.serve(Routes).pipe(
  ...
  Layer.provideMerge(
    NodeHttpServer.layer(createServer, { host: "127.0.0.1", port, gracefulShutdownTimeout: 0 }),
  ),
);
export const program = Effect.gen(function* () {
