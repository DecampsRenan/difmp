...
const bindingProblem = (
  ...
  if (actual !== criterion.hash) {
    return (
      `the text of ${criterion.id} does not match the hash frozen in the contract ` +
  ...
  if (!criterion.text.includes(projectName)) {
    return (
      `${criterion.id} does not mention the project name ${JSON.stringify(projectName)}, so ` +
  ...
  if (!uniquenessMarkers.some((marker) => normalised.includes(marker))) {
    return (
      `${criterion.id} is not a uniqueness statement (expected one of: ` +
...
export const projectUniqueInStorage: Check = async (ctx): Promise<CheckResult> => {
  const expected =
    "exactly one project with this attempt's name persisted in the server-side store";
  ...
  if (seedToken === undefined || seedToken.trim() === "") {
    return inconclusive(
      expected,
      `${seedTokenEnvVar} is not set, so the reserved probe cannot be called`,
    );
  }
