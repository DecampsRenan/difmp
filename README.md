...
# -> /tmp/decampsrenan-difmp-0.0.5.tgz
```
...
```sh
npm  i   -D /tmp/decampsrenan-difmp-0.0.5.tgz
pnpm add -D --allow-build=esbuild /tmp/decampsrenan-difmp-0.0.5.tgz
...
> local tarball by name and version, so `yarn add -D ./decampsrenan-difmp-0.0.5.tgz` after a
> rebuild silently reinstalls the previous bytes. `cp decampsrenan-difmp-0.0.5.tgz
