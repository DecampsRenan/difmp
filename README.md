...
the expectation, what was observed, the `method` (`model` or `code`), the evaluator, the confidence
the evaluator declared — when it declared one, and labelled as an observation that decided nothing —
and the `art_*` ids it relies on. A complete, unedited run directory for a failing case is committed at
...
# -> /tmp/decampsrenan-difmp-0.0.8.tgz
```
...
```sh
npm  i   -D /tmp/decampsrenan-difmp-0.0.8.tgz
pnpm add -D --allow-build=esbuild /tmp/decampsrenan-difmp-0.0.8.tgz
...
> local tarball by name and version, so `yarn add -D ./decampsrenan-difmp-0.0.8.tgz` after a
> rebuild silently reinstalls the previous bytes. `cp decampsrenan-difmp-0.0.8.tgz
...
- **A declared confidence decides nothing.** The evaluator reports a `confidence` in `[0, 1]`; it is
  recorded in `result.json` and printed on the report, and **no rule reads it**. There is no
  confidence threshold, and difmp will not gate a `passed` on one until that number has been shown
  to behave like a probability: a self-declared score is not a measurement, it is generated by the
  same pass as the verdict it grades, and the dangerous verdicts — a confidently hallucinated
  observation — are precisely the ones that come back high. It is collected so its calibration can
  be measured against the verdicts it accompanied; see [`docs/architecture.md`](docs/architecture.md)
  §4.
