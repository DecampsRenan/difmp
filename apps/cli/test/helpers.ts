...
export const allOutput = (result: ExecResult): string => [...result.stdout, ...result.stderr].join("\n")

/** A trivial page for the browser to land on. The scenarios never assert against its contents. */
export const startPage = async (): Promise<{ readonly url: string; readonly close: () => Promise<void> }> => {
