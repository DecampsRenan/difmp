...
describe("spec discovery", () => {
  ...
  it("is stably sorted: repeated discovery yields the identical order", async () => {
    ...
    expect([...runs[0]!]).toEqual([...runs[0]!].toSorted());
  });
  ...
  it("accepts a literal file and a directory argument", async () => {
    expect((await discover(["tests/beta.e2e.md"])).map((p) => relative(project, p))).toEqual([
      "tests/beta.e2e.md",
    ]);
    expect((await discover(["tests/nested"])).map((p) => relative(project, p))).toEqual([
  ...
  it("`difmp list` prints the same stable order and starts no browser", async () => {
    ...
    const parsed = JSON.parse(
      (await exec(["list", "--json"], { cwd: project })).stdout.join("\n"),
    ) as {
