    ...
    const body = await withServer(({ bus, url }) =>
      Effect.gen(function* () {
        ...
        const json = yield* Effect.promise(
          () => response.json() as Promise<{ accepted: boolean; reason: string }>,
        );
      ...
      const result = await withServer(({ bus, url }) =>
        Effect.gen(function* () {
          ...
          const escape = yield* Effect.promise(() =>
            fetch(`${url}api/artifacts/..%2f..%2fetc%2fpasswd`),
          );
          return {
            ok: ok.status,
            body: yield* Effect.promise(() => ok.text()),
