  ...
  Effect.gen(function* () {
    ...
    return yield* Effect.try({
      try: () => JSON.parse(raw) as unknown,
        ...
        new ExecutionError({
          message: `${file}: not valid JSON — ${cause instanceof Error ? cause.message : String(cause)}`,
        }),
    });
  });
...
export const loadReportInput = (
  ...
  Effect.gen(function* () {
    ...
    const contractExists = yield* fs
      .exists(layout.contract)
      .pipe(
        Effect.mapError(
          (cause) => new ExecutionError({ message: `${layout.contract}: ${cause.message}` }),
        ),
      );
    const contract = contractExists ? yield* decoded(layout.contract, decodeContract) : undefined;

    const inventoryExists = yield* fs
      .exists(layout.artifacts)
      .pipe(
        Effect.mapError(
          (cause) => new ExecutionError({ message: `${layout.artifacts}: ${cause.message}` }),
        ),
      );
