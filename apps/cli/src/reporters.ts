...
export const renderRunLines = (outcome: RunOutcome): ReadonlyArray<string> => {
  ...
  if (attempt !== undefined) {
    ...
    if (result.status !== "passed") {
      for (const criterion of attempt.criteria) {
        ...
        lines.push(
          `      ${criterion.criterionId} ${criterion.status}: ${truncate(criterion.expected, 100)}`,
        );
  ...
  if (result.status === "inconclusive") {
    lines.push(
      `      reason: ${result.reason}${result.detail === undefined ? "" : ` — ${result.detail}`}`,
    );
  }
  if (result.status === "error")
    lines.push(`      error at stage ${result.stage}: ${truncate(result.reason)}`);
  ...
  if (report !== undefined) {
    ...
    if (report.contract === undefined) {
      lines.push(
        "      ! the contract was never frozen — the report describes an infrastructure failure only",
      );
    }
    lines.push(
