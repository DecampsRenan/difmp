...
> =>
  Effect.gen(function* () {
    ...
    const note = (lines: ReadonlyArray<string>) =>
      Effect.forEach(lines, sink.note, { discard: true });
      ...
      Effect.gen(function* () {
        ...
        if (flags.ui) {
          ...
          if (server.host !== "127.0.0.1" && server.host !== "localhost" && server.host !== "::1") {
            yield* sink.note(
              `  ! --ui-host ${server.host} is NOT loopback: the dashboard is reachable from the network.`,
            );
          }
          yield* sink.note("");
...
const exitFor = (
  ...
  Effect.suspend((): Effect.Effect<void, Cancelled | ExecutionError | ScenariosNotPassing> => {
    ...
    if (failed + inconclusive > 0)
      return Effect.fail(new ScenariosNotPassing({ failed, inconclusive }));
    return Effect.void;
  });
