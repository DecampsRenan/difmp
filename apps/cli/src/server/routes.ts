...
export const makeRoutes = (options: RoutesOptions) => {
  ...
  const uiEvents = HttpRouter.add(
    ...
    Effect.gen(function*() {
      ...
      const body = sseStream(subscription, after, makeHarnessFrameEncoder(bus.harnessJournal, after))
      return HttpServerResponse.stream(body, { headers: sseHeaders })
    })
  )
  ...
  const contract = HttpRouter.add(
    ...
    Effect.gen(function*() {
      ...
      if (bytes === undefined) return HttpServerResponse.text("contract not frozen yet", { status: 404 })
  ...
  const artifacts = HttpRouter.add(
    ...
    Effect.gen(function*() {
      const run = bus.currentRun()
      if (run === undefined) return HttpServerResponse.text("no run yet", { status: 404 })
      const path = new URL((yield* HttpServerRequest.HttpServerRequest).url, "http://localhost").pathname
