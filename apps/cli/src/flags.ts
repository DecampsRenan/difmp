...
export const pathsArgument = Argument.String("paths").pipe(
  Argument.withDescription(
    "Scenario files, directories or globs (quote globs so the shell leaves them alone)",
  ),
  Argument.variadic(),
);
...
export const configFlag = Flag.String("config").pipe(
  ...
  Flag.withDescription(
    "Path to difmp.config.ts (default: nearest one above the working directory)",
  ),
  Flag.optional,
);
...
export const tagFlag = Flag.String("tag").pipe(
  Flag.withDescription(
    "Only scenarios carrying this tag. Repeatable; a scenario matches any of them.",
  ),
  Flag.atLeast(0),
);
