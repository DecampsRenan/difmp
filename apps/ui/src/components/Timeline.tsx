...
const FILTERS: ReadonlyArray<{
  ...
  readonly kinds: ReadonlyArray<TimelineKind>;
}> = [
...
export const Timeline = (props: { readonly entries: ReadonlyArray<TimelineEntry> }) => {
  ...
  const visible = useMemo(() => {
    ...
    const filtered =
      selected === undefined || selected.kinds.length === 0
        ? props.entries
        : props.entries.filter((entry) => selected.kinds.includes(entry.kind));
    return newestFirst ? [...filtered].toReversed() : filtered;
            ...
            >
              ...
              <span className="tl-body">
                ...
                {entry.detail === undefined ? null : (
                  <span className="tl-detail">{entry.detail}</span>
                )}
                {entry.outcome === undefined ? (
                  entry.kind === "action" ? (
                    <span className="tl-outcome pending">in progress…</span>
                  ) : null
                ) : (
