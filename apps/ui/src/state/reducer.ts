...
const mergeContract = (model: RunModel, contract: ContractView): RunModel => {
  ...
  return {
    ...
    ...(model.specPath === undefined && contract.specPath !== undefined
      ? { specPath: contract.specPath }
      : {}),
    ...(model.scenarioId === undefined && contract.id !== undefined
  ...
  switch (event.type) {
    ...
    case "contractFrozen": {
      ...
      return {
        ...row({
          ...
          detail: `${event.criterionIds.length} ${event.criterionIds.length === 1 ? "criterion" : "criteria"} · ${event.contractHash.slice(
            0,
            16,
          )}`,
        }),
    ...
    case "fixtureReady":
      return {
        ...row({
          kind: "lifecycle",
          tone: "neutral",
          label: `Fixture ready: ${event.fixtureName}`,
        }),
    ...
    case "browserContextOpened":
      return {
        ...row({
          kind: "lifecycle",
          tone: "neutral",
          label: "Browser context opened",
          detail: event.baseUrl,
        }),
    ...
    case "modelCallFinished": {
      ...
      return {
        ...
        model: {
          ...
          verifierTokens:
            base.model.verifierTokens + (event.role === "verifier" ? input + output : 0),
        },
      };
    }
    ...
    case "artifactAvailable": {
      ...
      const view: ArtifactView = {
        ...
        ...(owner === undefined
          ? {}
          : { actionId: owner.actionId, actionLabel: `${owner.actionId} · ${owner.tool}` }),
      };
      return {
        ...row({
          kind: "artifact",
    ...
    case "progressStalled":
      return {
        ...
        stalls: [
          ...base.stalls,
          { seq: event.seq, reason: event.reason, repeatedActions: event.repeatedActions },
        ],
      };
    ...
    case "error":
      return {
        ...row({
          ...
          detail:
            event.cause === undefined ? event.reason : `${event.reason} — ${truncate(event.cause)}`,
        }),
        errors: [
          ...base.errors,
          {
      ...
      return {
        ...row({
          ...
          detail: `${event.criteriaCount} ${event.criteriaCount === 1 ? "criterion" : "criteria"}${
            event.failedCriteria.length === 0
              ? ""
              : ` · failures: ${event.failedCriteria.join(", ")}`
          }`,
        }),
        criteria,
        status: event.status,
