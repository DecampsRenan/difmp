...
describe("language-model provider", () => {
  it.effect("reports a provider DEFECT as a typed ProviderError", () =>
    Effect.gen(function*() {
      ...
      const provider = yield* makeLanguageModelProvider({ id: "exploding", modelId: "exploding-v1" })
      const outcome = yield* Effect.result(
        provider.generate({ role: "browser", prompt: { messages: [{ role: "user", parts: [{ type: "text", text: "hi" }] }] } })
      )
