...
const textOf = (parts: ReadonlyArray<PromptPart>): string =>
  parts.filter((p): p is Extract<PromptPart, { readonly type: "text" }> => p.type === "text")
    .map((p) => p.text)
    .join("\n")
...
export const toAiPrompt = (prompt: HarnessPrompt): Prompt.Prompt => Prompt.make([...encodeMessages(prompt)])
      ...
      return {
        ...
        parts: message.content.flatMap((part): ReadonlyArray<PromptPart> =>
          part.type === "tool-result"
            ? [{ type: "toolResult", id: part.id, name: part.name, result: part.result, isError: part.isFailure }]
            : []
        )
      }
    }
