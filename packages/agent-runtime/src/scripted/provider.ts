...
const readConversation = (prompt: HarnessPrompt): {
  ...
  for (const message of prompt.messages) {
    for (const part of message.parts) {
      ...
      results.push({ id: part.id, name: part.name, result: part.result, isError: part.isError === true })
    ...
    Effect.gen(function*() {
      ...
      const verifierTurn = (options: LanguageModel.ProviderOptions) =>
        Effect.gen(function*() {
          ...
          const text = prompt.messages.flatMap((m) =>
            m.parts.flatMap((p) => p.type === "text" ? [p.text] : [])
          ).join("\n")
          const criterionId = readCriterionMarker(text)
          if (criterionId === undefined) {
            return yield* Effect.fail(
              scriptFailure("the scripted verifier could not find a `criterion_id:` marker in the prompt")
            )
          }
          ...
          const verdict = resolveEvidenceTokens(renderVerdict(script.verdicts ?? {}, criterionId, occurrence), available)
          return [
      ...
      const browsingTurn = (options: LanguageModel.ProviderOptions) =>
        Effect.gen(function*() {
          ...
          const turn = yield* Ref.modify(state, (s) => [s.turn, { ...s, turn: s.turn + 1 }] as const)
          ...
          for (const call of calls) {
            const id = yield* Ref.modify(state, (s) => [s.calls + 1, { ...s, calls: s.calls + 1 }] as const)
            const params = yield* Effect.try({
              try: () => resolveParams(call.params, ctx),
              catch: (cause) => scriptFailure(cause instanceof Error ? cause.message : String(cause))
            })
            parts.push({ type: "tool-call", id: `call_${id}`, name: call.tool, params })
          }
          parts.push(finishPart(calls.length === 0 ? "stop" : "tool-calls", step.usage === undefined ? usage : {
      ...
      const turn = (options: LanguageModel.ProviderOptions) => {
        seen?.push(options)
        return options.responseFormat.type === "json" ? verifierTurn(options) : browsingTurn(options)
      }
      ...
      return yield* LanguageModel.make({
        ...
        streamText: (options) =>
          Stream.unwrap(Effect.map(
            turn(options),
            (parts) => Stream.fromIterable(parts as ReadonlyArray<Response.StreamPartEncoded>)
          ))
      })
    })
  )
