...
export type ScriptedParams =
  | Record<string, unknown>
  | ((ctx: ScriptContext) => Record<string, unknown>);
...
export const resolveParams = (
  ...
  ctx: ScriptContext,
): Record<string, unknown> => (typeof params === "function" ? params(ctx) : params);
  ...
  observation?.elements.find(
    (element) =>
      ...
      (match.name === undefined ||
        ...
        element.placeholder === match.name),
  );
...
const requireLive = (ctx: ScriptContext, what: string): ObserveResult => {
  if (ctx.observation === undefined)
    throw new Error(`scripted step needs a live observation to ${what}`);
...
export const clickByName = (
  name: string,
  options?: { readonly role?: string; readonly intent?: string },
) => ({
  tool: "click" as const,
