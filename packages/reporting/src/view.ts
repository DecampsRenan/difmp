...
export const criterionStatusLabel = (status: CriterionStatus): string =>
  criterionStatusLabels[status];
...
export const downgradeLabel = (reason: CriterionDowngrade["reason"]): string =>
  downgradeLabels[reason];
...
const describeEvent = (
  ...
  switch (event.type) {
    case "runStarted":
      return {
        title: "Run started",
        fields: [field("scenario", event.scenarioId), field("spec", event.specPath)],
      };
    case "configResolved":
      return {
    ...
    case "contractFrozen":
      return {
        ...
        fields: [
          field("hash", event.contractHash),
          field("criteria", event.criterionIds.join(", ")),
        ],
      };
    case "fixtureReady":
    ...
    case "actionStarted":
      return {
        ...
        fields: [
          field("id", event.actionId),
          field("intent", event.intent),
          field("params", event.params),
        ],
      };
    case "actionFinished":
      return {
    ...
    case "evidenceRequested":
      return {
        ...
        fields: [
          field("criterion", event.criterionId),
          field("by", event.requestedBy),
          field("note", event.note),
        ],
      };
    ...
    case "actionGuidanceExceeded":
      return {
        title: "Indicative action threshold crossed",
        fields: [field("count", event.rendering)],
      };
    case "budgetExhausted":
      return {
        title: `Blocking budget exhausted: ${event.budget}`,
        fields: [
          field("used", event.used),
          field("limit", event.limit),
    ...
    case "error":
      return {
        ...
        fields: [
          field("reason", event.reason),
          field("fatal", event.fatal),
          field("cause", event.cause),
        ],
      };
    case "cancellationRequested":
      return {
        title: "Cancellation requested",
        fields: [field("reason", event.reason), field("source", event.source)],
      };
    case "runFinished":
      return {
        title: `Run finished — ${event.status}`,
        fields: [
          field("criteria", event.criteriaCount),
          field("failed", event.failedCriteria.join(", ")),
        ],
      };
  }
};
...
const budgetLines = (
  ...
  return [
    ...
    },
    {
      ...
      unit: "tokens",
      note:
        `of which ${attempt.model.verifierTokens} for the verifier; the verifier reserve ` +
...
const collectDiagnostics = (
  ...
  if (input.contract === undefined) {
    out.push({
      ...
      source: "contract",
      message:
        "The contract was never frozen: the run stopped before step 3 of §6 " +
  ...
  if (!input.finalized) {
    out.push({
      ...
      source: "journal",
      message:
        "The journal does not end on an intact `runFinished`: the run was not finalized " +
  ...
  for (const criterion of criteria) {
    for (const downgrade of criterion.downgrades) {
      out.push({
        ...
        message: `Status "${criterionStatusLabel(downgrade.from)}" brought down to "${criterionStatusLabel(
          downgrade.to,
        )}": ${downgrade.detail}`,
      });
    }
    ...
    if (criterion.limitations !== undefined) {
      out.push({
        severity: "info",
        source: `limitation ${criterion.id}`,
        message: criterion.limitations,
      });
    }
    if (criterion.hashMismatch) {
      out.push({
        ...
        message:
          `The evaluated hash (${criterion.resultHash ?? "absent"}) does not match the contract hash ` +
...
export const buildReportView = (input: ReportInput): ReportView => {
  ...
  for (const attempt of result.attempts) {
    for (const criterion of attempt.criteria) {
      criterionResults.set(criterion.criterionId, {
        result: criterion,
  ...
  const criteria = (contract?.criteria ?? []).map((criterion: Criterion): CriterionView => {
    ...
    return {
      ...
      contractHash,
      hashMismatch:
        found !== undefined && contractHash !== "" && found.result.criterionHash !== contractHash,
