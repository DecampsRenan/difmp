    ...
    const writeAtomic = (target: string, content: string) =>
      Effect.gen(function*() {
        ...
        yield* fs.makeDirectory(path.dirname(target), { recursive: true }).pipe(
          Effect.mapError((cause) => fail(`could not create ${path.dirname(target)}: ${cause.message}`))
        )
        yield* fs.writeFileString(temp, content).pipe(
          Effect.mapError((cause) => fail(`could not write ${temp}: ${cause.message}`))
        )
        yield* fs.rename(temp, target).pipe(
          Effect.mapError((cause) => fail(`could not replace ${target}: ${cause.message}`))
        )
      })

    const report = (input: ReportInput): Effect.Effect<ReadonlyArray<ReportOutput>, ReporterError> =>
      Effect.gen(function*() {
        const rendered = yield* Effect.try({
          try: () => renderAll(input),
          catch: (cause) => fail(`rendering failed: ${cause instanceof Error ? cause.message : String(cause)}`)
        })
...
): Layer.Layer<Reporter, never, FileSystem.FileSystem | Path.Path> => Layer.effect(Reporter, make(kinds))

export const jsonReporterLayer: Layer.Layer<Reporter, never, FileSystem.FileSystem | Path.Path> = fileReporterLayer([
  "json"
])
export const junitReporterLayer: Layer.Layer<Reporter, never, FileSystem.FileSystem | Path.Path> = fileReporterLayer([
  "junit"
])
export const htmlReporterLayer: Layer.Layer<Reporter, never, FileSystem.FileSystem | Path.Path> = fileReporterLayer([
  "html"
])
