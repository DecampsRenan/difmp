...
const renderToDisk = (name: FixtureName, outputDir: string): Promise<Rendered> => {
  ...
  }).pipe(Effect.provide(fileReporterLayer().pipe(Layer.provide(platform))), Effect.runPromise);
};
...
describe.each(fixtureNames)("standalone report for %s", (name: FixtureName) => {
  ...
  it("states that a model evaluation is probabilistic, and only for model criteria", async () => {
    ...
    const deterministic = notes.filter(
      (n) => n.includes("deterministic") && !n.includes("probabilistic"),
    );
    const criteria = input.contract?.criteria ?? [];
    expect(probabilistic.length).toBeGreaterThanOrEqual(
      criteria.filter((c) => c.method === "model").length,
    );
    expect(deterministic.length).toBeGreaterThanOrEqual(
      criteria.filter((c) => c.method === "code").length,
    );
  });
  ...
  it("links artifacts as relative paths that resolve next to the report", async () => {
    const input = loadFixture(name, outputDir);
    const linkable = input.inventory.artifacts.filter(
      (a) => a.state === "present" && a.path !== undefined,
    );
    ...
    for (const href of hrefs) {
      ...
      expect(readFileSync(join(rendered.root, decodeURIComponent(href)), "utf8")).toContain(
        "demo content",
      );
    }
  });
  ...
  it("carries an inert, parseable data island rather than executable state", async () => {
    const scripts = await page
      .locator("script")
      .evaluateAll((nodes) => nodes.map((n) => (n as HTMLScriptElement).type));
    expect(scripts).toEqual(["application/json"]);
    const parsed = await page.evaluate(
      () =>
        JSON.parse(document.getElementById("harness-report-data")!.textContent!) as {
          runId: string;
        },
    );
