...
describe("report view", () => {
  ...
  it("reports budgets as consumed and remaining", () => {
    ...
    expect(budgets.find((b) => b.key === "maxModelCalls")).toMatchObject({ used: 6, limit: 6, remaining: 0 })
    expect(budgets.find((b) => b.key === "maxTokens")).toMatchObject({ used: 32290, limit: 40000, remaining: 7710 })
  })

  it("preserves individual criterion statuses when the aggregate is error", () => {
    const view = buildReportView(loadFixture("error"))
    expect(view.status).toBe("error")
    expect(view.criteria.map((c) => [c.id, c.status])).toEqual([["c1", "failed"], ["c2", "pending"]])
  })
  ...
  it("records which branch of the absence rule produced a verdict", () => {
    expect(buildReportView(loadFixture("inconclusive")).criteria[0]!.absence).toBe("uncertain-navigation")
    expect(buildReportView(loadFixture("failed-persistence")).criteria[1]!.absence).toBe("established-at-checkpoint")
  })
})
...
describe("HTML escaping", () => {
  ...
  it("escapes the hostile project name coming from the scenario", () => {
    ...
    expect(html).toContain("&lt;img src=x onerror=&quot;document.title=&#39;PWNED-NAME&#39;&quot;&gt;")
  })
...
describe("cancellation", () => {
  ...
  const cancelled = decodeResult({
    ...
    attempts: [{
      ...
      criteria: attempt.criteria.map((c) => ({ ...c, status: "pending", observed: "", evidence: [] })),
