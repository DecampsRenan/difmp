...
const executeRaw = (options: RunOptions) =>
  Effect.gen(function* () {
    ...
    const fixtureLayer =
      options.fixtureSetupError === undefined
        ? fixtures.layer
        : Layer.succeed(
            FixtureManager,
            FixtureManager.of({
...
const execute = (options: RunOptions) =>
  executeRaw(options).pipe(
    Effect.map((raw): RunOutput => {
      if (Result.isFailure(raw.outcome)) {
        throw new Error(
          `expected the run to settle, it failed with: ${raw.outcome.failure.message}`,
        );
      }
...
describe("runner", () => {
  it.effect("runs the reference scenario end to end and passes", () =>
    Effect.gen(function* () {
      ...
      expect(out.result.attempts[0]!.criteria.map((c) => c.status)).toEqual([
        "passed",
        "passed",
        "passed",
      ]);
  ...
  it.effect("freezes the contract before opening the browser context", () =>
    Effect.gen(function* () {
      ...
      const contract = JSON.parse(
        yield* fs.readFileString(`${out.runRoot}/contract.json`).pipe(Effect.orDie),
      ) as {
  ...
  it.effect("a 40-action run still passes and warns exactly once", () =>
    Effect.gen(function* () {
      const loaded = yield* expectSuccess(spec("project-create.e2e.md"));
      const turns: Array<ScriptedTurn> = Array.from({ length: 40 }, () => ({
        toolCalls: [observe],
      }));
  ...
  it.effect("a blocking budget ends the loop as inconclusive, never failed", () =>
    Effect.gen(function* () {
      const loaded = yield* expectSuccess(spec("project-create.e2e.md"));
      const turns: Array<ScriptedTurn> = Array.from({ length: 20 }, () => ({
        toolCalls: [observe],
      }));
      const out = yield* execute({
        spec: loaded,
  ...
  it.effect("rejects a stale observation reference without touching another element", () =>
    Effect.gen(function* () {
      ...
      const failedAction = out.events.find(
        (e) => e.type === "actionFinished" && e.outcome === "error",
      );
      ...
      expect(
        out.events.some(
          (e) =>
            ...
            e.stage === "evidence" &&
            !e.fatal &&
            e.reason.includes("could not be recorded"),
        ),
      ).toBe(true);
    }).pipe(Effect.provide(platform)),
  );
