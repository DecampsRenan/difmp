...
const queuedVerifier = (
  queues: Readonly<Record<string, ReadonlyArray<Verdict>>>,
  fallback: Verdict,
) => {
    ...
    Verifier.of({
      ...
      verify: (request) =>
        Effect.sync((): VerificationResponse => {
          ...
          const verdict =
            queue === undefined ? fallback : (queue[Math.min(index, queue.length - 1)] ?? fallback);
          return {
            outcome: {
              _tag: "verdict",
              result: {
                ...
                evidence:
                  verdict.evidence ?? request.evidence.map((e: EvidenceItem) => e.artifactId),
...
const browserLayer = (options: BrowserOptions = {}) => {
  ...
  const session: BrowserSession = {
    ...
    finalize: () =>
      Effect.succeed([
        { kind: "trace", state: "present", path: "/tmp/trace.zip" } as CaptureOutcome,
      ]),
  };
  return Layer.succeed(
    BrowserDriver,
    BrowserDriver.of({ id: "fake", openContext: () => Effect.succeed(session) }),
  );
};
...
const execute = (options: RunOptions) =>
  Effect.gen(function* () {
    ...
    const result = yield* runScenario({
      ...
      registries: {
        fixtures: makeRegistry<Fixture>("fixture", {
          "authenticated-workspace": async () => ({ public: {} }),
        }),
...
describe("verdict integrity — a re-`check` never upgrades a decided criterion", () => {
  it.effect("a `failed` criterion stays failed when the agent asks again and gets `passed`", () =>
    Effect.gen(function* () {
      ...
      expect(c1.reChecks![0]).toMatchObject({
        status: "passed",
        applied: false,
        requestedBy: "agent",
      });
...
describe("evidence integrity applies to `method: code` exactly as it does to `model`", () => {
  ...
  it.effect("a check citing an artifact that does not exist cannot pass", () =>
    Effect.gen(function* () {
      ...
      expect(c2.downgrades![0]).toMatchObject({
  ...
  it.effect("a failure resting on an absence after uncertain navigation is downgraded", () =>
    Effect.gen(function* () {
      ...
      const out = yield* execute({
        ...
        fallback: {
          status: "failed",
          observed: "I do not see the project",
  ...
  it.effect("an absence established on a settled, observed page still fails", () =>
    Effect.gen(function* () {
      ...
      const out = yield* execute({
        ...
        fallback: {
          status: "failed",
          observed: "absent after the reload",
...
describe("a failure to persist mandatory evidence can never end in a silent success", () => {
  it.effect(
    "a criterion whose checkpoint capture failed is not `passed`, and the run is an error",
    () =>
      Effect.gen(function* () {
        const loaded = yield* expectSuccess(spec("project-create.e2e.md"));
        const out = yield* execute({
...
describe("known secrets never reach the contract, the journal or the manifest", () => {
  it.effect(
    "a secret a fixture leaked into `public` is redacted everywhere it would be written",
    () =>
      Effect.gen(function* () {
        const loaded = yield* expectSuccess(spec("project-create.e2e.md"));
        const out = yield* execute({
