...
describe("maxActions guidance is indicative, never blocking", () => {
  ...
  it("renders the threshold the way the spec asks for", () => {
    ...
    expect(recordAction({ ...state, notified: false }).notice?.rendering).toBe(
      "29 actions / 25 suggested",
    );
  });
...
describe("blocking budgets are separate and end the loop as inconclusive", () => {
  ...
  it("withholds the verifier reserve from the browser loop only", () => {
    ...
    const state = chargeTokens(makeBudgetState(0), "browser", {
      inputTokens: 800,
      outputTokens: 0,
    });
  ...
  it("keeps the verifier reserve available after an oversized browsing turn overshot it", () => {
    ...
    const overshot = chargeTokens(makeBudgetState(0), "browser", {
      inputTokens: 1200,
      outputTokens: 0,
    });
    expect(tokensUsed(overshot)).toBeGreaterThan(budgets.maxTokens);
    expect(canStartModelCall({ budgets, state: overshot, role: "browser", nowMs: 0 })._tag).toBe(
      "deny",
    );
    expect(canStartModelCall({ budgets, state: overshot, role: "verifier", nowMs: 0 })._tag).toBe(
      "allow",
    );
...
describe("aggregation order", () => {
  ...
  it("a blocking execution error wins over a failed criterion", () => {
    const verdict = aggregate({
      executionError: { stage: "evidence", reason: "disk full" },
      criteria: failing,
    });
...
describe("navigation origin allow-list", () => {
  ...
  it.effect("accepts an allowed origin and rejects everything else", () =>
    Effect.gen(function* () {
      expect(
        yield* expectSuccess(checkNavigationOrigin("http://127.0.0.1:3000/app", allowed)),
      ).toBe("http://127.0.0.1:3000/app");
      const foreign = yield* expectFailure(
        checkNavigationOrigin("https://evil.example/x", allowed),
      );
