...
const wire = (options: Wiring) =>
  Effect.gen(function* () {
    ...
    const dir = yield* fs
      .makeTempDirectoryScoped({ prefix: "difmp-lifecycle-" })
      .pipe(Effect.orDie);
    ...
    const run = runScenario({
      ...
      Effect.provide(
        Layer.mergeAll(
          ...
          options.verifierLayer ??
            scriptedVerifier({}, options.fallbackVerdict ?? { status: "passed" }),
    ...
    return {
      ...
      result: readJson(`${runRoot}/result.json`).pipe(
        Effect.map((v) => v as RunResult | undefined),
      ),
...
describe("cancellation and lifecycle", () => {
  ...
  it.live(
    "an interrupted run still writes result.json, journals runFinished and reports cancelled",
    () =>
      Effect.gen(function* () {
  ...
  it.live(
    "a cooperative cancellation aborts the in-flight model call instead of waiting for it",
    () =>
      Effect.gen(function* () {
        const loaded = yield* expectSuccess(spec("project-create.e2e.md"));
        const seen = { aborted: false, calls: 0 };
  ...
  it.live("a model call is bounded by budgets.operationTimeoutMs", () =>
    Effect.gen(function* () {
      ...
      const result = yield* wired.run.pipe(
        Effect.timeoutOrElse({
          duration: "5 seconds",
  ...
  it.live("fixture setup is bounded by budgets.fixtureSetupTimeoutMs", () =>
    Effect.gen(function* () {
      ...
      const wired = yield* wire({
        spec: loaded,
        fixtures: { "authenticated-workspace": async () => ({ public: {} }) },
        fixtureLayer: Layer.succeed(
          FixtureManager,
          FixtureManager.of({ setup: () => Effect.never }),
        ),
      ...
      if (result.status === "inconclusive") expect(result.reason).toBe("budget-exhausted");
      const events = yield* wired.journal;
      expect(events.find((e) => e.type === "budgetExhausted")).toMatchObject({
  ...
  it.live(
    "after a budget is exhausted no action or model call runs, but the final capture may",
    () =>
      Effect.gen(function* () {
        const loaded = yield* expectSuccess(spec("project-create.e2e.md"));
        const wired = yield* wire({
    ...
    Effect.gen(function* () {
      ...
      const events = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          Effect.provide(RunStore.layer({ runId, outputDir: dir }).pipe(Layer.provide(platform))),
        ),
      );
      expect(yield* PubSub.isShutdown(events)).toBe(true);
      yield* PubSub.awaitShutdown(events).pipe(
        Effect.timeoutOrElse({
          duration: "1 second",
    ...
    Effect.gen(function* () {
      const project = yield* expectSuccess(
        resolveConfig({
          source: "difmp.config.ts",
          config: {
            budgets: { fixtureSetupTimeoutMs: 1_000, maxIdleTurns: 5, maxEvidenceRequests: 3 },
          },
        }),
      );
