  ...
  it.effect("tolerates a truncated final line and reports the run as not finalised", () =>
    withStore((store, fs) =>
      Effect.gen(function*() {
        ...
        yield* fs.writeFileString(store.layout.events, `{"schemaVersion":1,"seq":2,"runId":"${runId}"`, {
          flag: "a"
        })
        const journal = yield* readRunJournal(store.layout.events)
  ...
  it.effect("replaces result.json atomically and leaves no temp file behind", () =>
    withStore((store, fs) =>
      Effect.gen(function*() {
        ...
        const written = JSON.parse(yield* fs.readFileString(store.layout.result)) as { durationMs: number }
