      ...
      Object.prototype.hasOwnProperty.call(entries, name)
        ...
        : Effect.fail(
            new RegistryError({
              kind,
              name,
              reason: "is not registered in difmp.config.ts",
              registered: names,
            }),
          ),
  };
  ...
  if (typeof entries !== "object" || entries === null || Array.isArray(entries)) {
    return Effect.fail(
      new RegistryError({
        kind,
        name: `${kind}s`,
        reason: "must be an object mapping names to functions",
        registered: [],
      }),
    );
  }
    ...
    if (typeof value !== "function") {
      return Effect.fail(
        new RegistryError({
          ...
          registered: Object.keys(record).toSorted(),
        }),
      );
    }
  }
