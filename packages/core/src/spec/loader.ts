...
const inlineScalarLocator =
  (valuePos: Position): SourceLocator =>
  (relLine, relColumn) =>
    relLine === 1
...
export const parseSpec = (input: {
  ...
  Effect.gen(function* () {
    ...
    if (rawVersion !== undefined && rawVersion !== SupportedSpecVersion) {
      ...
      return yield* fail(
        specPath,
        `unsupported spec version ${JSON.stringify(rawVersion)} (supported: ${SupportedSpecVersion})`,
        {
          field: "version",
    ...
    const frontmatter = yield* decodeFrontmatter(raw).pipe(
      Effect.catchTag("SchemaError", (error) => {
        ...
        const pos =
          firstField === undefined || firstField === "" ? undefined : positionOf(firstField);
    ...
    const hasVerification =
      frontmatter.verification !== undefined && frontmatter.verification.trim() !== "";
    ...
    if (hasVerification) {
      ...
      locate = rawValue.startsWith("|")
        ? blockScalarLocator(anchor, rawValue)
        : inlineScalarLocator(anchor);
    } else {
      text = section!.text;
    ...
    for (const boundId of Object.keys(checks)) {
      if (!knownIds.has(boundId)) {
        ...
        return yield* fail(
          specPath,
          `checks references criterion ${boundId}, but this spec only has ${criteria.length} criteria (${criteria
            .map((c) => c.id)
            .join(", ")})`,
...
export class SpecLoader extends Context.Service<
  SpecLoader,
  {
    ...
    readonly loadAll: (
      specPaths: ReadonlyArray<string>,
    ) => Effect.Effect<ReadonlyArray<LoadedSpec>, SpecError>;
  }
      ...
      const load = Effect.fn("SpecLoader.load")(function* (specPath: string) {
        const content = yield* fs
          ...
          .pipe(
            Effect.mapError(
              (cause) =>
                new SpecError({ specPath, reason: `cannot read spec file: ${cause.message}` }),
            ),
          );
