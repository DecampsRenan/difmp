 ...
 * mandatory excludes.
 */
export const discoverSpecs = (
  options: DiscoverOptions,
): Effect.Effect<ReadonlyArray<string>, SpecError> =>
  Effect.tryPromise({
    try: async () => {
      ...
      const searched =
        requested.length === 0
          ...
          : [
              ...globs,
              ...literals.filter((p) => !p.endsWith(".e2e.md")).map((p) => `${p}/**/*.e2e.md`),
            ];
      ...
      const runNamed = async (): Promise<ReadonlyArray<string>> =>
        namedFiles.length === 0
          ? []
          : glob(namedFiles.map(absolute), { ...shared, cwd: options.cwd });

      const [searchedFiles, explicitFiles] = await Promise.all([runSearch(), runNamed()]);
      return [...new Set([...searchedFiles, ...explicitFiles].map((f) => resolve(f)))].toSorted();
    },
    catch: (cause) =>
      new SpecError({
