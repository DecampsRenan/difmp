...
export const aggregate = (input: AggregationInput): AggregateOutcome => {
  ...
  if (input.executionError !== undefined) {
    return { status: "error", stage: input.executionError.stage, reason: input.executionError.reason }
  }
  ...
  if (unresolved.length > 0) {
    const reason: InconclusiveReason = input.budgetExhausted === true
      ? "budget-exhausted"
      : unresolved.every((c) => c.status === "inconclusive")
      ? "insufficient-evidence"
      : "unresolved-criteria"
    const detail = input.budgetExhausted === true && input.budgetDetail !== undefined
      ? input.budgetDetail
