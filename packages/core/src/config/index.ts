...
export const resolveConfig = (options: {
  ...
  Effect.gen(function*() {
      ...
      Effect.catchTag("SchemaError", (error) =>
        Effect.fail(
          new ConfigInvalidError({
            source,
            problems: schemaProblems(error).map((p) => (p.path === "" ? p.message : `${p.path}: ${p.message}`))
          })
        ))
    )
    ...
    if (baseOrigin === undefined) {
      return yield* Effect.fail(toConfigError([`baseUrl: ${JSON.stringify(decoded.baseUrl)} is not an absolute URL`]))
    }
    const badOrigins = decoded.allowedOrigins.filter((origin) => originOf(origin) === undefined)
    if (badOrigins.length > 0) {
      return yield* Effect.fail(
        toConfigError(badOrigins.map((o) => `allowedOrigins: ${JSON.stringify(o)} is not an absolute origin`))
      )
    }
...
export const resolveInputPrecedence = (
  ...
  Effect.suspend(() => {
    const declared = new Set([...Object.keys(options.configInputs), ...Object.keys(options.specInputs)])
    const undeclared: Array<string> = []
    for (const key of Object.keys(options.fileInputs ?? {})) {
      if (!declared.has(key)) undeclared.push(`--inputs-file declares "${key}", which is not an input of the config or the spec`)
    }
    for (const key of Object.keys(options.cliInputs ?? {})) {
      if (!declared.has(key)) undeclared.push(`--input declares "${key}", which is not an input of the config or the spec`)
    }
