...
export const CaptureConfig = Schema.Struct({
  ...
  screenshots: ScreenshotPolicy.pipe(Schema.withDecodingDefaultKey(Effect.succeed(defaultCapture.screenshots))),
  retainTraceOn: TraceRetention.pipe(Schema.withDecodingDefaultKey(Effect.succeed(defaultCapture.retainTraceOn)))
...
export const ResolvedConfig = Schema.Struct({
  ...
  providerOptions: withDefault(Schema.Record(Schema.String, Schema.Unknown), defaultConfig.providerOptions),
