...
export const RunId = Schema.String.check(Schema.isPattern(/^r_[a-z2-7]{13}$/)).annotate({
  identifier: "RunId",
});
export const AttemptId = Schema.String.check(Schema.isPattern(/^a[1-9][0-9]*$/)).annotate({
  identifier: "AttemptId",
});
export const ActionId = Schema.String.check(Schema.isPattern(/^act_[0-9]+$/)).annotate({
  identifier: "ActionId",
});
export const ObservationId = Schema.String.check(Schema.isPattern(/^obs_[0-9]+$/)).annotate({
  identifier: "ObservationId",
});
export const ArtifactId = Schema.String.check(Schema.isPattern(/^art_[0-9]+$/)).annotate({
...
export const makeRunId: Effect.Effect<RunId, PlatformError, Crypto.Crypto> = Effect.gen(
  function* () {
