...
const known = (scope: InterpolationScope): string => {
  const inputKeys = Object.keys(scope.inputs).toSorted();
          ...
          if (mode === "validate") {
            ...
            return raise(
              `the fixture does not expose a public value named "${key}" (exposed: ${
                Object.keys(options.scope.fixture).toSorted().join(", ") || "none"
              })`,
            );
          }
        } else {
    ...
    for (const [key, value] of Object.entries(options.declared)) {
      ...
      for (const ref of refs) {
        ...
        if (ref.expression.startsWith("fixture.")) {
          return yield* raise(
            "inputs are resolved before the fixture runs and may not reference fixture values",
          );
        }
