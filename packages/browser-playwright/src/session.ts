...
/** Runs a Playwright call under the caller's abort signal, tagging any rejection with its operation. */
  ...
  Effect.gen(function* () {
      ...
      state.closed
        ? Effect.fail(
            browserError(
              operation,
              "closed",
              "the browser context is closed; no further action is possible",
            ),
          )
        : Effect.void;
    ...
    const didNavigate = (before: number, probeMs: number): Effect.Effect<boolean, BrowserError> =>
      Effect.gen(function* () {
        ...
        // `navigations` is incremented by the `framenavigated` listener registered above, not by
        // this loop — polling it is the whole point. The rule only looks inside the loop body.
    ...
    const observe = (observationId: string): Effect.Effect<ObserveResult, BrowserError> =>
      guarded(
        ...
        Effect.gen(function* () {
          ...
          const snapshot = yield* attempt("observe", (signal) =>
            page.ariaSnapshot({ mode: "ai", timeout, signal }),
          );
    ...
    const navigate = (params: {
      readonly url: string;
    }): Effect.Effect<NavigateResult, BrowserError> =>
      guarded(
        "navigate",
        Effect.gen(function* () {
          ...
          yield* attempt("navigate", (signal) =>
            page.goto(params.url, { waitUntil: "commit", timeout, signal }),
          );
          const settled = yield* attempt("navigate", () =>
            page.waitForLoadState("load", { timeout }).then(
              () => true,
              () => false,
            ),
          );
    ...
    const click = (params: { readonly observationId: string; readonly ref: string }) =>
      guarded(
        "click",
        Effect.flatMap(resolveRef("click", params.observationId, params.ref), (locator) =>
    ...
    const fill = (params: {
      ...
      guarded(
        "fill",
        Effect.flatMap(resolveRef("fill", params.observationId, params.ref), (locator) =>
          interaction("fill", (signal) => locator.fill(params.value, { timeout, signal }), 0),
        ),
      );
      ...
      guarded(
        ...
        Effect.gen(function* () {
            ...
            if (params.observationId === undefined) {
              return yield* Effect.fail(
                browserError(
                  "press",
                  "stale-reference",
                  "a ref can only be used together with its observationId",
                ),
              );
            }
          ...
          if (
            ...
            current?.observationId !== params.observationId
          ) {
            return yield* Effect.fail(
              browserError(
                "press",
    ...
    const writeLog = (
      ...
      attempt("write-log", async () => {
        ...
        Effect.catch((error: BrowserError) =>
          Effect.succeed(outcome(kind, "failed", { reason: error.message })),
        ),
      );

    const finalize = (finalizeOptions: {
      readonly retainTrace: boolean;
    }): Effect.Effect<ReadonlyArray<CaptureOutcome>> =>
      lock
        .withPermits(1)(
          Effect.gen(function* () {
            ...
            if (options.capture.trace === "on") {
              ...
              if (stopped._tag === "Failure") {
                ...
                captures.push(
                  outcome("trace", "missing", {
                    reason: 'discarded: capture.retainTraceOn is "failure" and the attempt passed',
                  }),
                );
              } else {
                captures.push(
                  outcome("trace", "present", { path: tracePath, bytes: stopped.success }),
                );
              }
            }

            // 2. Closing the CONTEXT is what finalises the video; browser.close() alone leaves 0 bytes.
            const contextClosed = yield* attempt("context-close", () => context.close()).pipe(
              Effect.result,
            );
            ...
            captures.push(
              yield* writeLog(
                "console-log",
                join(options.attemptDir, "console.jsonl"),
                recorders.console,
              ),
            );
            captures.push(
              yield* writeLog(
                "network-log",
                join(options.attemptDir, "network.jsonl"),
                recorders.network,
              ),
            );
            ...
            if (options.capture.video === "on") {
              if (video === null) {
                captures.push(
                  outcome("video", "missing", { reason: "the page produced no video handle" }),
                );
