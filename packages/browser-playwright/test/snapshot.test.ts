...
describe("parseAiSnapshot", () => {
  it("keeps only referenced nodes, with role, name and level", () => {
    ...
    expect(elements.map((e) => e.ref)).toEqual(["e2", "e3", "e4", "e5", "e6", "e7", "e8", "e9", "e10", "e12"])
    expect(elements[1]).toEqual({ ref: "e3", role: "heading", name: "Sign in to Projects", level: 1 })
    expect(elements.find((e) => e.ref === "e7")).toEqual({ ref: "e7", role: "textbox", name: "Email" })
  })
